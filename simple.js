...
function handleLoginRequest(req, res) {
    if (req.method === 'GET') {
        ...
        if (name && email) {
            ...
            res.write(`Hello ${name}, your email id ${email} has been registered successfully at NIHA BAKES`);
        } else {
    ...
    if (req.method === 'POST') {
        ...
        req.on('end', () => {
            ...
            if (name && email && password) {
                res.writeHead(200, { 'Content-Type': 'text/plain' });
                res.write(`Hello ${name}, you have successfully signed up with email id ${email} at NIHA BAKES`);
            } else {
                res.writeHead(400, { 'Content-Type': 'text/plain' });
